...
class VideoControlButton extends React.PureComponent<
  ControlButtonProps,
  ControlButtonState
> {
  ...
  render() {
    return (
      <VrButton
        style={[
          this.props.style,
          { backgroundColor: this.state.isFocused ? "#444" : "#222" }
        ]}
...
class VideoSliderBar extends React.PureComponent<SliderBarProps> {
  ...
  _onEmptyMove = (e: Object) => {
    this._gazedPosition =
      this.props.progress + (1 - this.props.progress) * e.nativeEvent.offset[0];
  };
...
class VideoControl extends React.PureComponent<VideoControlProps> {
  ...
  _registerEventListener(
    prePlayer: ?VideoPlayerInstance,
    nextPlayer: ?VideoPlayerInstance
  ) {
    ...
    return (
      ...
      >
        ...
        <View style={styles.timerContainer}>
          ...
          >
            {`${videoTimeFormat(this.state.position)}/${videoTimeFormat(
              this.state.duration
            )}`}
          </Text>
        </View>
